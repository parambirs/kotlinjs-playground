...
abstract class Shape() {
    ...
    // these two abstract methods define that our shapes can be dragged
    ...
    fun CanvasRenderingContext2D.shadowed(
        shadowOffset: Vector,
        alpha: Double,
        render: CanvasRenderingContext2D.() -> Unit
    ) {
        save()
...
class Logo(override var pos: Vector) : Shape() {
    ...
    // get-only properties like this save you lots of typing and are very expressive
    val position: Vector
...
class Creature(override var pos: Vector, val state: CanvasState) : Shape() {
    ...
    val relSize = 0.05
    // these properties have no backing fields and in java/js they could be represented as little helper functions
    val radius: Double
    ...
    fun getGradient(context: CanvasRenderingContext2D): CanvasGradient {
        ...
        val gradient = context.createRadialGradient(gradientCentre.x, gradientCentre.y, 1.0,
            gradientCentre.x, gradientCentre.y, 2 * radius)
